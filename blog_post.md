...
-   We also define the encryption, network isolation, and VPC
    ...
    preferences
    [here](https://docs.aws.amazon.com/sagemaker/latest/dg/interface-vpc-endpoint.html).
    If you are using the default VPC and security groups then you can
    leave these configuration parameters empty, see example in this
    [configuration
...
Here is an example of an inference request and response from the real
time endpoint using the implementation above:

**Inference request format** (view and change this example as you would
like for your custom use case)
