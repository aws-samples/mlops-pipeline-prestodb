...
We have now seen an end-to-end process of our solution, from fetching
data by connecting to a PrestoDB instance, followed by training,
evaluating, registering the model. We then approved the latest
registered model from our training pipeline solution and ran batch
inference against batch data stored in S3. Furthermore, we deployed the
latest approved model as a real time SageMaker endpoint to run
inferences. The results are given below in the `Results` section.
